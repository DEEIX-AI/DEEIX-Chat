...
For a checkout-based setup, run the following Docker commands from the repository root. A no-repository deployment can place the required Compose and config files in any directory; the full and VoceChat profiles use published images and do not need the source tree. All root compose profiles expose the app at `http://localhost:8080` by default and mount `config.yaml` to `/app/config.yaml` inside the container.
...
curl -fsSL https://raw.githubusercontent.com/amaoworks/DEEIX-Chat/v0.4.1-4/docker-compose.full.yml -o docker-compose.full.yml
curl -fsSL https://raw.githubusercontent.com/amaoworks/DEEIX-Chat/v0.4.1-4/config.full.example.yaml -o config.full.example.yaml
cp config.full.example.yaml config.yaml
docker compose -f docker-compose.full.yml up -d
```
...
Compose profiles pin the application image to the release that contains the current UI and API, `ghcr.io/amaoworks/deeix-chat:v0.4.1-4`. Do not replace it with an older `latest` image when enabling internal messaging; the app and VoceChat bundle must be from a compatible release. Compose files reference an image and do not define a build step. Build a local image first, then select it with `DEEIX_CHAT_IMAGE`:
