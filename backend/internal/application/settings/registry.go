...
var settingSpecs = []settingSpec{
	// 站内消息配置
	{Namespace: "internal_messaging", Key: "enabled", ValueType: "bool", Default: "false", Description: "是否启用站内私聊",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.InternalMessagingEnabled }, toBool)},
	{Namespace: "internal_messaging", Key: "max_file_bytes", ValueType: "int", Default: "20971520", Description: "单个聊天文件大小上限（字节）",
		Validate: intRange(1<<20, 100<<20), Apply: applyField(func(c *config.Config) *int64 { return &c.InternalMessagingMaxBytes }, toInt64)},
	{Namespace: "internal_messaging", Key: "retention_days", ValueType: "int", Default: "0", Description: "消息保留天数，0 表示永久保留",
		Validate: intRange(0, 3650), Apply: applyField(func(c *config.Config) *int { return &c.InternalMessagingKeepDays }, toInt)},
	{Namespace: "internal_messaging", Key: "user_quota_bytes", ValueType: "int", Default: "0", Description: "每位用户发送文件的总配额（字节），0 表示不限制",
		Validate: int64Range(0, 1<<50), Apply: applyField(func(c *config.Config) *int64 { return &c.InternalMessagingUserQuota }, toInt64)},
	{Namespace: "internal_messaging", Key: "browser_notifications", ValueType: "bool", Default: "true", Description: "是否允许用户开启浏览器消息通知",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.InternalMessagingWebNotify }, toBool)},
