...
type KnowledgeBaseUserSharePanelProps = {
  knowledgeBasePublicID: string;
};

export function KnowledgeBaseUserSharePanel({ knowledgeBasePublicID }: KnowledgeBaseUserSharePanelProps) {
  const t = useTranslations("knowledgeBases.userShare");
  ...
  const refresh = React.useCallback(async () => {
    if (!featurePolicy.resourceSharingEnabled) {
      return;
    }
    ...
    setLoading(true);
    try {
      setEntries(await listKnowledgeBaseACL(token, knowledgeBasePublicID));
    } catch {
      toast.error(t("loadFailed"));
    } finally {
      setLoading(false);
    }
  }, [featurePolicy.resourceSharingEnabled, knowledgeBasePublicID, t]);
  ...
  const handleGrant = async () => {
    ...
    setWorking(true);
    try {
      await grantKnowledgeBaseACL(token, knowledgeBasePublicID, username.trim(), role);
      setUsername("");
      await refresh();
  ...
  const handleRevoke = async (granteeUserID: number) => {
    ...
    setWorking(true);
    try {
      await revokeKnowledgeBaseACL(token, knowledgeBasePublicID, granteeUserID);
      await refresh();
  ...
  return (
    <div className="mt-4 space-y-3 rounded-lg border border-border/50 p-3">
